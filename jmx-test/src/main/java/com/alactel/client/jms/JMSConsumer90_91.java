...
public class JMSConsumer90_91 {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			// ServerLocatorImpl sl = (ServerLocatorImpl)
			// srcCF.getServerLocator();
			// Field fname =
			// ServerLocatorImpl.class.getField("discoveryGroupConfiguration");
			// fname.setAccessible(true);

			// fname.set(sl, new
			// DiscoveryGroupConfiguration("dg-group1",5432,5432,new
			// UDPBroadcastGroupConfiguration("231.7.7.7",9876,"192.168.1.140",-1)));
			// TransportConfiguration[] tc =
			// srcCF.getServerLocator().getStaticTransportConfigurations();
			...
			// HornetQClient.createServerLocatorWithHA(server1, server2);
			//
			// final HornetQConnectionFactory srcCF = new
			// org.hornetq.jms.client.HornetQConnectionFactory(locator);

			srcCF.setClientID("jms-client-1");
			srcCF.setInitialConnectAttempts(-1);
			...
			srcConnect.setFailoverListener(new FailoverEventListener() {
				...
				public void failoverEvent(FailoverEventType paramFailoverEventType) {
					...
					try {
						...
						if ("135.252.247.90".equals(ip)) {
							sf.getConnectorConfiguration().getParams().put("host", "135.252.247.91");
							srcConnect.setFailoverListener(this);
						} else if ("192.168.1.90".equals(ip)) {
							sf.getConnectorConfiguration().getParams().put("host", "192.168.1.91");
							srcConnect.setFailoverListener(this);
						} else if ("135.252.247.91".equals(ip)) {
							sf.getConnectorConfiguration().getParams().put("host", "135.252.247.90");
							srcConnect.setFailoverListener(this);
						} else if ("192.168.1.91".equals(ip)) {
							sf.getConnectorConfiguration().getParams().put("host", "192.168.1.90");
							srcConnect.setFailoverListener(this);
						} else {
							// ignore
						}
